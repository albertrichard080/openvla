...
# Define the scaling factor (assumes the model's output is in centimeters)
SCALE_FACTOR = 0.01
    ...
    def timer_callback(self):
        with self.action_lock:
            if self.action is not None:
                # Convert action to meters
                converted_action = [coord * SCALE_FACTOR for coord in self.action]
                # Keep quaternion unchanged (if already normalized)
               # quaternion = self.action[3:]
            #    converted_action = scaled_action + quaternion

                # Prepare and publish the message
                msg = Float64MultiArray()
                msg.data = converted_action
                self.publisher_.publish(msg)
                self.get_logger().info(f'Published action: {converted_action}')
            else:
